				 ...
				 unsigned int *nr_realms)
{
	...
	uint64_t zone_size, next = 0, lba = zbc_dev_sect2lba(dev, sector);
	size_t bufsz = ZBC_RPT_REALMS_HEADER_SIZE;
	unsigned int i, nr = 0, remaining = 0, desc_len;
	unsigned int page_descs, to_process;
	struct zbc_sg_cmd cmd;
	...
	if (dev->zbd_info.zbd_flags & ZBC_STANDARD_RPT_REALMS) {
		...
		do {
			...
			/* Descriptors available in this page */
			page_descs = (cmd.bufsz -
				      ZBC_RPT_REALMS_HEADER_SIZE) /
				     desc_len;
			to_process = remaining < page_descs ?
				     remaining : page_descs;

			/* Get zone realm descriptors */
			buf += ZBC_RPT_REALMS_HEADER_SIZE;
			for (i = 0; i < to_process; i++, realms++) {
				realms->zbr_number =
					zbc_ata_get_dword(buf);
				realms->zbr_restr =
					zbc_ata_get_word(&buf[4]);
				...
				/* FIXME don't use nr_domains, use
				 * desc_len to limit iteration */
...
static int zbc_ata_get_zoned_device_info(struct zbc_device *dev)
{
	...
	if (!ver || ver == 0xffff) {
		...
		case ZBC_ATA_ZAC2_REV1B:
			...
			/*
			 * The standard REPORT REALMS layout appeared in ZDRev4,
		...
		case ZBC_ATA_ZAC2_REV12:
			zbc_debug("%s: ZAC minor version is ZAC-2 Rev. 12\n",
				  dev->zbd_filename);
			di->zbd_flags |= ZBC_ZONE_OP_COUNT_SUPPORT;
			di->zbd_flags |= ZBC_STANDARD_RPT_REALMS;
			break;
 ...
 */
static int zbc_ata_do_report_zones(struct zbc_device *dev, uint64_t sector,
				...
				struct zbc_zone *zones, unsigned int *nr_zones)
{
	...
	return zbc_ata_do_report_zones(dev, sector, ro, NULL, zones, nr_zones,
				       bufsz);
}
