				  ...
				  unsigned int *nr_realms)
{
	...
	do {
		...
		/* First page: get total count and validate */
		if (!nr) {
			...
			/* Cap to caller's buffer size */
			if (nr > *nr_realms)
				nr = *nr_realms;

			if (!(dev->zbd_info.zbd_flags &
			      ZBC_STANDARD_RPT_REALMS)) {
				zbc_error("%s: REPORT REALMS is not"
					  " supported by device",
					  dev->zbd_filename);
				ret = -ENXIO;
				zbc_sg_cmd_destroy(&cmd);
		...
		/* Determine how many descriptors are in this page */
		page_descs = (cmd.bufsz - ZBC_RPT_REALMS_HEADER_SIZE)
			     / desc_len;
		...
		for (i = 0; i < to_process; i++, realms++) {
			...
			/* FIXME don't use nr_domains, use desc_len
			 * to limit iteration */
		...
		/* Done with this page's command */
		zbc_sg_cmd_destroy(&cmd);

		/* Set up LBA for next page */
		lba = next;

	} while (next != 0 && remaining > 0);

	/* Update nr to reflect total descriptors filled */
	nr -= remaining;
